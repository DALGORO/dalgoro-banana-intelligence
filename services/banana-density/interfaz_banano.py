...
BUILD_ID = "DALGORO_BANANA_GUI_V1_20260718_R4_REPARACION"
...
def critical_pipeline_config(
    config: dict[str, Any],
) -> dict[str, Any]:
    """Extrae los parámetros críticos usados por el orquestador."""
    ...
    return {
        ...
        "yolo_iou": float(parameters.get("yolo_iou", 0.70)),
        "yolo_imgsz": int(parameters.get("yolo_imgsz", 640)),
        "yolo_device": str(parameters.get("yolo_device", "auto")),
        "max_detections": int(
            parameters.get("max_detections", 1000)
        ),
        "deduplication_distance_m": float(
            parameters.get("deduplication_distance_m", 1.00)
        ),
        "spatial_analysis_config": support_configs["spatial_analysis"],
    }


def pipeline_config_fingerprint(
    config: dict[str, Any],
) -> str:
    """Calcula la misma huella SHA-256 usada por el orquestador."""

    encoded = json.dumps(
        critical_pipeline_config(config),
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
...
def repair_resume_snapshot(
    run_directory: Path,
) -> tuple[Path, str | None]:
    """Repara una ejecución R3 que guardó el marcador como capa real."""

    snapshot_yaml = run_directory / "configuracion_analisis.yaml"
    snapshot_json = run_directory / "configuracion_analisis.json"
    state_path = run_directory / "estado_pipeline.json"

    if not snapshot_yaml.is_file():
        raise FileNotFoundError(
            "La ejecución no contiene configuracion_analisis.yaml."
        )

    config = read_yaml(snapshot_yaml)
    analysis = config.get("analysis")
    if not isinstance(analysis, dict):
        raise ValueError("La configuración histórica no contiene analysis.")

    exclusions_gpkg = analysis.get("exclusions_gpkg")
    exclusions_layer = analysis.get("exclusions_layer")
    if not exclusions_gpkg:
        return snapshot_yaml, None

    source_path = normalize_existing_file(str(exclusions_gpkg))
    if not source_path.is_file():
        raise FileNotFoundError(
            f"No existe el GeoPackage registrado: {source_path}"
        )

    layers = list_gpkg_feature_layers(source_path)
    if not layers:
        raise ValueError("El GeoPackage no contiene capas espaciales.")

    if (
        exclusions_layer != ALL_EXCLUSION_LAYERS
        and exclusions_layer in layers
    ):
        return snapshot_yaml, None

    generated_path, generated_layer = build_combined_exclusions(
        source_path=source_path,
        layer_names=layers,
        farm_name=str(analysis.get("farm_name", run_directory.name)),
    )
    analysis["exclusions_gpkg"] = str(generated_path)
    analysis["exclusions_layer"] = generated_layer

    atomic_write_yaml(snapshot_yaml, config)
    atomic_write_json(snapshot_json, config)

    message = (
        "Se reparó la configuración heredada. Se utilizará la capa "
        f"'{generated_layer}' de {generated_path}."
    )

    if state_path.is_file():
        state = json.loads(state_path.read_text(encoding="utf-8"))
        if not isinstance(state, dict):
            raise ValueError("estado_pipeline.json no es válido.")
        state["critical_config_fingerprint"] = pipeline_config_fingerprint(config)
        ...
        *,
        ...
        try:
            if mode == "resume":
                ...
                snapshot, repair_message = (
                    repair_resume_snapshot(
                        resume_directory
                    )
                )
