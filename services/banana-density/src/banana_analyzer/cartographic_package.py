...
def load_config(config_path: str | Path | None) -> tuple[Path, dict[str, Any]]:
    ...
    if config_version < REQUIRED_CONFIG_VERSION:
        raise ValueError(
            "Se requiere config/cartography.yaml versión 2 o superior. "
            f"Versión encontrada: {config_version}. "
            "Descargue cartography_DALGORO_v2.yaml y renómbrelo como cartography.yaml."
        )
    ...
    config = loaded["cartography"]
    branding = config.get("branding")
    if not isinstance(branding, dict):
        raise ValueError(
            "La configuración cartográfica versión 2 debe contener "
    ...
    if missing_branding:
        raise ValueError(
            "Faltan claves de identidad gráfica en cartography.branding: "
...
def configure_brand_font(config: dict[str, Any]) -> str:
    """Usa Montserrat cuando está instalada; no incorpora fuentes al proyecto."""

    preferred = str(config.get("branding", {}).get("font_family", "Montserrat"))
    try:
...
def add_brand_header(
    ...
    """Agrega el encabezado corporativo y el logo sin deformarlo."""

    branding = config.get("branding", {})
    primary = branding.get("primary_color", "#21393F")
    accent = branding.get("accent_color", "#B57548")
    text_color = branding.get("secondary_color", "#192B2F")

    figure.add_artist(
        Rectangle((0, 0.982), 1, 0.018, transform=figure.transFigure, color=primary, zorder=200)
    )
    figure.add_artist(
        Rectangle((0, 0.885), 1, 0.005, transform=figure.transFigure, color=accent, zorder=200)
    )

    logo_path = resolve_asset_path(branding.get("logo_path"))
    if logo_path is not None:
        logo_axis = figure.add_axes([0.018, 0.895, 0.205, 0.078], zorder=210)
        logo_axis.imshow(plt.imread(str(logo_path)))
        logo_axis.axis("off")
...
def read_raster_preview(
    ...
    with rasterio.open(raster_path) as source:
        ...
        rgb = np.moveaxis(raster_float.filled(np.nan), 0, -1)
...
def add_footer(
    ...
    """Agrega pie corporativo, autoría y advertencia técnica."""
    ...
    figure.add_artist(
        Rectangle((0, 0), 1, 0.047, transform=figure.transFigure, color=primary, zorder=200)
    )
    figure.add_artist(
        Rectangle((0, 0.047), 1, 0.004, transform=figure.transFigure, color=accent, zorder=201)
    )
    figure.text(
        0.014,
        0.022,
        f"Elaborado por: {author} | Escenario: {target_density_plants_ha:g} plantas/ha",
        ha="left",
        va="center",
        fontsize=6.8,
        color="white",
...
def create_base_figure(
    ...
    """Crea una página cartográfica A4 horizontal con identidad DALGORO."""
    ...
    axis = figure.add_axes([0.055, 0.075, 0.89, 0.795])

    axis.imshow(raster_rgba, extent=raster_extent, origin="upper", zorder=1)
    ...
    axis.set_xlabel("Coordenada Este (m)", fontsize=8)
    axis.set_ylabel("Coordenada Norte (m)", fontsize=8)
    axis.tick_params(labelsize=7)
    axis.grid(False)

    add_brand_header(figure, title, subtitle, config)
...
def generate_cartographic_package(
    ...
    try:
        ...
        brand_font = configure_brand_font(cartography_config)
        ...
        result.metadata = {
            ...
            "config_version": int(cartography_config.get("_config_version", 0)),
            ...
            "branding_logo": str(resolve_asset_path(cartography_config.get("branding", {}).get("logo_path"))) if resolve_asset_path(cartography_config.get("branding", {}).get("logo_path")) else None,
        }
