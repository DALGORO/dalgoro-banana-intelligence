    ...
    def scope_hierarchy_matches(
        ...
        *,
        ...
        self.events.append(
            (
                "scope_hierarchy",
                tuple(sorted(farm_scopes, key=lambda value: str(value.farm_id))),
                tuple(sorted(plot_scopes, key=lambda value: str(value.plot_id))),
            )
        )
...
def validate_scope_hierarchy_rejections() -> None:
    actor_id = uuid4()
    target_id = uuid4()
    actor = _actor()
    farm_id = uuid4()
    ...
    create_repository = _FakeRepository(
        {actor_id: _state(actor_id, actor)},
        hierarchy_matches=False,
    )
    _assert_conflict(
        lambda: DBIAdminService(create_repository).guard_membership_create(
            actor,
            requested,
            actor_membership_id=actor_id,
        )
    )
    assert [event[0] for event in create_repository.events] == [
        "lock_and_load",
        "scope_hierarchy",
    ]

    before = _snapshot(permissions=frozenset({DBIPermission.READ}))
    mutation_repository = _FakeRepository(
        {
            actor_id: _state(actor_id, actor),
    ...
    _assert_conflict(
        lambda: DBIAdminService(mutation_repository).mutate_membership(
            actor,
            before,
            requested,
            actor_membership_id=actor_id,
            ...
            correlation_ref="scope-conflict-001",
        )
    )
    assert [event[0] for event in mutation_repository.events] == [
        "lock_and_load",
        "scope_hierarchy",
    ]
...
def validate_state_and_version_conflicts() -> None:
    actor_id = uuid4()
    target_id = uuid4()
    actor = _actor()
    before = _snapshot(permissions=frozenset({DBIPermission.READ}))
...
def validate_mutation_order_and_no_op() -> None:
    actor_id = uuid4()
...
def main() -> None:
    ...
    validate_scope_hierarchy_rejections()
    validate_membership_change_and_last_admin()
    validate_state_and_version_conflicts()
    validate_mutation_order_and_no_op()
